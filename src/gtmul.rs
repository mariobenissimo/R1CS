// proving i know CT which equals T^c
// CT = input
// c T = witness 
...
impl<I, IV> Clone for MulGtVerificaion<I, IV>
where
    I: Pairing,
    IV: PairingVar<I>,
{
...
impl<I, IV> ConstraintSynthesizer<<I as Pairing>::BaseField> for MulGtVerificaion<I, IV>
where
    I: Pairing,
    IV: PairingVar<I>,
    ...
    IV::GTVar: FieldVar<I::TargetField, I::BaseField>,
{
    fn generate_constraints(
        ...
        let t_var = IV::GTVar::new_witness(cs.clone(), || Ok(self.t.unwrap().0))?;

        let scalar_in_fq =
            &I::BaseField::from_bigint(<I::BaseField as PrimeField>::BigInt::from_bits_le(
                self.c.unwrap().into_bigint().to_bits_le().as_slice(),
            ))
            .unwrap();
        let c_var = FpVar::new_witness(cs.clone(), || Ok(scalar_in_fq))?;

        let bits_c = c_var.to_bits_le()?;
        let exp = t_var.pow_le(&bits_c)?;
        ct_var.enforce_equal(&exp)?;
...
mod tests {
    ...
    fn preimage_constraints_correctness() {
        ...
        MulGtVerificaion::<I, IV>::new(&mut rng)
...
mod test_groth {
    ...
    fn test_prove_and_verify() {
        let mut rng = ark_std::test_rng();
        let circuit = MulGtVerificaion::<I, IV>::new(&mut rng);
